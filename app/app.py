...
st.markdown("""
ホテル業界7年の実務経験から、清掃スタッフの人員配置は
現場感覚に頼ることが多く、人員過不足による
業務の滞り（清掃の遅れ・手待ち時間の発生）という
課題がありました。
本ダッシュボードでは、予約データを可視化することで
効率的なシフト設計の根拠を提供することを目的としています。
""")
st.caption("※本データは実務経験をもとに作成した架空のデータです。料金・客層傾向は地方の小規模ホテルの一般的な水準を参考にしています。")
...
- 通常期は客層により1-2名で調整
→ ビジネス客中心の日はチェックアウトが早いので清掃が短時間で済むため 
- 週次で予約状況を確認し柔軟にシフト調整することを推奨
→予約状況は1週間単位で比較的に安定しており、シフト設計の精度を保ちやすいため
""")